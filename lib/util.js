...
function stats (opts) {
    ...
    console.log('Finished:', number(opts.ms), 'ms', '|', number(opts.rows), 'rows', '|', number(opts.bytes), 'bytes', '|', number(bps(opts.ms, opts.bytes)), 'bytes per second', '|', number(rps(opts.ms, opts.rows)), 'rows per second');
